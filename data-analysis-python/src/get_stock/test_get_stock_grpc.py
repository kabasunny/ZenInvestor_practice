    ...
    def test_get_stock_data(self):  # 実際のテストメソッド
        ...
        with grpc.insecure_channel(
            ...
            response = stub.GetStockData(
                get_stock_pb2.GetStockRequest(ticker="^GSPC")
            )  # リクエストを送信
            print(
                "gPRCレスポンスは Protocol Buffers の形式:", response
            )  # レスポンスをターミナルに表示

            # 数値データの検証
            self.assertIn(
                "Close", response.stock_data
            )  # レスポンスに"Close"キーが含まれていることを確認
            self.assertIn(
                "Open", response.stock_data
            )  # レスポンスに"Open"キーが含まれていることを確認
            self.assertIn(
                "High", response.stock_data
            )  # レスポンスに"High"キーが含まれていることを確認
            self.assertIn(
                "Low", response.stock_data
            )  # レスポンスに"Low"キーが含まれていることを確認
            self.assertIn(
                "Volume", response.stock_data
            )  # レスポンスに"Volume"キーが含まれていることを確認

            # 日付データの検証
            self.assertIsInstance(
                response.date, str
            )  # レスポンスの日付が文字列であることを確認
            print(
                f"取得データはPython の辞書形式: {response.stock_data}, 日付: {response.date}"
            )
