    ...
    def test_handle_generate_chart_request(self):
        ...
        for percent in percentages:
            ...
            indicator = generate_chart_pb2.IndicatorData(
                type=f"Indicator_{percent}", values=indicator_values
            )
        ...
        # チャートデータをファイルに書き込む
        with open(
            f"{output_dir}/service_genarate_chart.png", "wb"
        ) as f:  # ファイル名を変更
            f.write(chart_data)

        self.assertTrue(chart_data)  # チャートデータが存在することを確認
        print(f"Chart saved as {output_dir}/service_genarate_chart.png")
