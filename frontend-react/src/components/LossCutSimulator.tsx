...
const LossCutSimulator: React.FC = () => {
  ...
  const [simulationDate, setSimulationDate] = useState("2024-01-10");
  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.post(
        "http://localhost:8086/losscutSimulation",
        {
          ticker,
          simulationDate,
          stopLossPercentage,
          ...
          <div className="mb-4">
            ...
            <p
              className={`mt-4 text-2xl ${
                profitLoss < 0 ? "text-red-500" : "text-teal-500"
              }`}
            >
              損益率: {profitLoss}%
            </p>
        ...
        <div>
          <div className="mb-4">
            <label className="block mb-2">
              ストップロスパーセンテージ: {stopLossPercentage}%
            </label>
            <input
              type="range"
              min="1"
              ...
              onChange={(e) =>
                setStopLossPercentage(parseFloat(e.target.value))
              }
              className="w-full"
            />
          ...
          <div className="mb-4">
            <label className="block mb-2">
              トレーリングストップトリガー: {trailingStopTrigger}%
            </label>
            <input
              type="range"
              min="5"
              ...
              onChange={(e) =>
                setTrailingStopTrigger(parseFloat(e.target.value))
              }
              className="w-full"
            />
            <div className="flex justify-between text-xs">
          ...
          <div className="mb-4">
            <label className="block mb-2">
              トレーリングストップ更新: {trailingStopUpdate}%
            </label>
            <input
              type="range"
              min="1"
              ...
              onChange={(e) =>
                setTrailingStopUpdate(parseFloat(e.target.value))
              }
              className="w-full"
            />
            <div className="flex justify-between text-xs">
