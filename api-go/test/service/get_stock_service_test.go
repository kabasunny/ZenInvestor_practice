...
func TestGetStockDataIntegration(t *testing.T) {
	...
	assert.NotEmpty(t, res.StockName) // レスポンスに銘柄名が含まれていることを検証
	...
	// 銘柄名を書き込み
	if _, err := file.WriteString(fmt.Sprintf("Stock Name: %s\n", res.StockName)); err != nil {
		t.Errorf("failed to write stock name to output file: %v", err)
	}
