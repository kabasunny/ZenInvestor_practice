// api-go\test\service\gateway\client\get_stock_data_with_dates_client_test.go
package client_test
...
func TestNewStockDataWithDatesClient(t *testing.T) {
	client_test_helper.LoadTestEnv() // ヘルパー関数を呼び出す

	ctx := context.Background()
	stockClient, err := client.NewGetStockDataWithDatesClient(ctx)
	assert.NoError(t, err)
	assert.NotNil(t, stockClient)
	defer stockClient.Close() // 接続を閉じる
}

func TestGetStockDataWithDates(t *testing.T) {
	client_test_helper.LoadTestEnv() // ヘルパー関数を呼び出す
	ctx := context.Background()
	stockClient, err := client.NewGetStockDataWithDatesClient(ctx)
	assert.NoError(t, err)
	...
	req := &get_stock_data_with_dates.GetStockDataWithDatesRequest{
		Ticker:    "AAPL",
	...
	filename := fmt.Sprintf("get_stock_data_with_dates_client_test%s.txt", timestamp)
...
// go test -v ./test/service/gateway/client/get_stock_data_with_dates_client_test.go
