...
type LosscutSimulatorService interface {
	GetStockChartForLCSim(ctx context.Context, ticker string, simulationDate string, stopLossPercentage, trailingStopTrigger, trailingStopUpdate float64) (*generate_chart_lc_sim.GenerateChartLCResponse, float64, error)
}
